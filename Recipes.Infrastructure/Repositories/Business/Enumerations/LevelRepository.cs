...
public class LevelRepository : EfRepositoryBase<Level>, ILevelRepository
{
    public LevelRepository(RecipesDbContext db) : base(db) { }

    public Task AddAsync(Level e, CancellationToken token)
    ...
    public async Task<(IReadOnlyList<Level> Items, int Total)> GetPageAsync(string? q, int page, int size, CancellationToken token)
    {
        page = page <= 0 ? 1 : page;
        size = size <= 0 ? 20 : size;
