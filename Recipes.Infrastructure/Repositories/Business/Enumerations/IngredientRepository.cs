...
public class IngredientRepository : EfRepositoryBase<Ingredient>, IIngredientRepository
{
    public IngredientRepository(RecipesDbContext db) : base(db) { }

    public Task AddAsync(Ingredient e, CancellationToken token)
    ...
    public async Task<(IReadOnlyList<Ingredient>, int)> GetPageAsync(string? q, int page, int size, CancellationToken token)
    {
        page = page <= 0 ? 1 : page;
        size = size <= 0 ? 20 : size;
