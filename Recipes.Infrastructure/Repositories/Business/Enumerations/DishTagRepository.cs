...
public class DishTagRepository : EfRepositoryBase<DishTag>, IDishTagRepository
{
    public DishTagRepository(RecipesDbContext db) : base(db) { }

    public Task AddAsync(DishTag e, CancellationToken token)
    ...
    public Task<IReadOnlyList<DishTag>> GetByIdsAsync(IEnumerable<int> ids, CancellationToken token)
        => Set.Where(a => ids.Contains(a.Id)).ToListAsync(token)
        .ContinueWith(t => (IReadOnlyList<DishTag>)t.Result, token);

    public async Task<(IReadOnlyList<DishTag>, int)> GetPageAsync(string? q, int page, int size, CancellationToken token)
    {
        page = page <= 0 ? 1 : page;
        size = size <= 0 ? 20 : size;
